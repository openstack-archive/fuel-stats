...
def get_oswls(master_node_uid):
    ...
    oswls_count = _get_db_objs_data(OSWL, sql_clauses,
                                    (OSWL.id.asc(),), paging_params)
    oswls_data = _get_db_objs_count(OSWL, sql_clauses)
...
@bp.route('/installation_info/filtered', methods=['GET'])
def get_filtered_installation_info():
    paging_params = get_paging_params()
