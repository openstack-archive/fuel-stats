...
def _get_db_objs(model, sql_clauses, order_by, paging_params):
    """Gets DB objects by sql_clauses
    :param model: DB model
    :param sql_clauses: collection of clauses for selecting DB objects
    :param order_by: ordering for DB objects
    :param paging_params: dictionary with limit, offset values
    :return: generator on jsonyfied DB objects data
    """
    result = db.session.query(model).filter(and_(*sql_clauses)).\
        order_by(order_by).limit(paging_params['limit']).\
...
def get_oswls(master_node_uid):
    ...
    oswls_data = _get_db_objs(OSWL, sql_clauses, OSWL.id.asc(),
                              paging_params)
...
def get_action_logs(master_node_uid):
    paging_params = get_paging_params()
    app.logger.debug("Fetching action_logs for: %s, paging params",
                     master_node_uid, paging_params)
    sql_clauses = (AL.master_node_uid == master_node_uid,)
    action_logs_data = _get_db_objs(AL, sql_clauses, AL.id.asc(),
                                    paging_params)
    app.logger.debug("Action_logs for: %s, paging params: %s fetched",
                     master_node_uid, paging_params)
    return Response(action_logs_data, mimetype='application/json')
