...
def get_resources_types():
    ...
    :return: tuple of resources types names
    """
...
def archive_dir(dir_path):
    ...
    archive = zipfile.ZipFile(tmp_file, 'w', zipfile.ZIP_DEFLATED)
    for root, dirs, files in os.walk(dir_path):
        for f in files:
            archive.write(os.path.join(root, f), arcname=f)
    app.logger.debug("Dir '%s' archiving finished", dir_path)
    archive.close()
    return archive
...
def all_reports():
    ...
    def cleanup(response):
        ...
        app.logger.debug("Removing temporary archive %s", archive.filename)
        os.unlink(archive.filename)
        app.logger.debug("Request all_reports handled")
        return response

    name = 'reports_from{}_to{}.zip'.format(get_from_date(), get_to_date())
